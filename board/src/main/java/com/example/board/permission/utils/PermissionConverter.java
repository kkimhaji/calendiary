...
//            return new byte[0];
//        }
//        // 역순으로 저장된 기존 권한 비트열 고려
//        String reversedPermissions = new StringBuilder(permissions).reverse().toString();
//
//        int length = reversedPermissions.length();
//        byte[] bytes = new byte[(length + CHUNK_SIZE - 1) / CHUNK_SIZE];
//
//        for (int i = 0; i < length; i++) {
//            if (reversedPermissions.charAt(i) == '1') {
//                int byteIndex = i / CHUNK_SIZE;
//                int bitIndex = i % CHUNK_SIZE;
//                bytes[byteIndex] |= (1 << bitIndex);
//            }
