...
public interface DiaryRepository extends JpaRepository<Diary, Long> {
    ...
    @Query("SELECT d FROM Diary d LEFT JOIN FETCH d.author WHERE d.title LIKE %:keyword% AND d.author.id = :memberId")
    Page<Diary> searchByTitle(
            @Param("keyword") String keyword,
            @Param("memberId") Long memberId,
            Pageable pageable);

    @Query("SELECT d FROM Diary d LEFT JOIN FETCH d.author WHERE d.content LIKE %:keyword% AND d.author.id = :memberId")
    Page<Diary> searchByContent(
            @Param("keyword") String keyword,
            @Param("memberId") Long memberId,
            Pageable pageable);

    @Query("SELECT d FROM Diary d LEFT JOIN FETCH d.author WHERE (d.title LIKE %:keyword% OR d.content LIKE %:keyword%) AND d.author.id = :memberId")
