...
//    List<Team> findAllByMember(Member member);
    @Query("SELECT new com.example.board.dto.TeamInfoDTO(" +
            "t.id, t.name, t.createdBy.nickname, t.createdAt, " +
            "(SELECT COUNT(tm) FROM TeamMember tm WHERE tm.team = t)) " +
            "FROM Team t " +
            "WHERE t.id = :teamId")
    TeamInfoDTO findTeamDetailsById(@Param("teamId") Long teamId);
}
