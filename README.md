...
`byte[]` 비트맵 기반으로 권한을 표현하여 저장 공간과 조회 비용을 줄이는 대신,
권한 종류가 증가할수록 비트 위치 관리와 권한 매핑에 대한 복잡성이 증가하는 구조적 한계가 있습니다.

또한 일반적인 Boolean 필드나 Enum 기반 표현보다 비트 연산의 의도를 파악하기 어려워,
권한 상수 및 헬퍼 메서드를 통한 명시적인 관리가 필요합니다.
...
일정 시점에 DB에 일괄 반영하는 방식을 적용하여 개별 조회마다 발생하는 DB UPDATE를 줄였습니다.

대신 메모리에 누적된 조회수는 서버 재시작 시 DB에 반영되지 않은 상태라면 유실될 수 있으며,
서버를 다중 인스턴스로 확장할 경우 인스턴스별 캐시가 분리되어 조회수 정합성 문제가 발생할 수 있습니다.

향후 다중 인스턴스 환경에서는 Redis와 같은 외부 저장소 기반의 분산 캐시 구조로 전환할 필요가 있습니다.
